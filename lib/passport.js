...
module.exports = function (app) {
        ...
        process.nextTick(function () {
            ...
            userDb.findOne({ userId: profile.emails[0].value }, function (error, users) {
                if (users) {
                    ...
                    var user = new userDb({ id: shortid.generate(), userId: profile.emails[0].value, nickname : profile.displayName, verification : 1 , freeflag : 0})
    ...
    passport.use(new KakaoStrategy({
        clientID: key.clientID2,
        callbackURL: 'http://119.18.120.225:4000/auth/kakao/callback'
    },
        function (accessToken, refreshToken, profile, done) {
            ...
            userDb.findOne({ userId: _profile.kaccount_email }, function (error, users) {
                if (users) {
                    ...
                    var user = new userDb({ id: _profile.id, userId: _profile.kaccount_email, nickname: _profile.properties.nickname, verification:1,freeflag:0 })
                    if(!(_profile.kaccount_email))
                    {
                        return done(null, false, {
                            message: '이메일 허용을 동의 하지 않으면 카카오 계정으로 로그인을 사용할 수 없습니다.'
                        });
                    }
                    else{
                        user.save(function (error, data) {
    ...
    app.get('/auth/kakao', passport.authenticate('kakao', {
        failureRedirect: '/fail2',
        // failureRedirect: 'http://119.18.120.225:3000',
        failureFlash: true,
