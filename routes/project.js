   ...
   */
    ...
    projectDb.find().countDocuments(
      function (error, count) {
          projectDb.find({ dueDate: { '$lt': Date.now() } }).countDocuments(
              function (error, count2) {
                  Obj.flag = "success"
                  Obj.ingCount = count - count2
                  Obj.doneCount = count2
                  userDb.find({ freeflag: 1 }).countDocuments(function (error, count3) {
                      Obj.freeCount = count3
                      return response.send(Obj)
                  })
              }
          )
